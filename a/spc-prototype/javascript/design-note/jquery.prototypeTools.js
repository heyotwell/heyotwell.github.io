...
2. DYNAMIC STATES allow your page to dynamically respond to different states. This is the main purpose of this plugin.
	...
	Inside document.ready(), call $.prototyper.respondTo() for each state you want to configure:
	
	$.prototyper.respondTo({
		state: "admin",
	...
	* "response" is a function to run when the page is loaded in this state. For example, show/hide elements, set default form field values
	...
	Note that $.prototyper.respondTo() builds your navigation postit links in the order you call respondTo().
	...
	$.prototyper = {
		...
		createStateList: function(contentArray) {
			...
			for(i=0;i < this.states.length; i++){
				if (this.states[i].state == "default") {
					...
					$("ul", $statelist).append("<li><p>" + this.states[i].text + "</p></li>");
				} else {
					//normal state links
			...
			//fade statelist when page is scrolled, 
			//this is pure eye candy :-)
			// $(window).scroll(function() {
			// 			if($(window).scrollTop() > 10) {
			// 				$("div.statelist").fadeOut("fast");
			// 			} else {
			// 				$("div.statelist").fadeIn("fast");
			// 			}
			// 		});
		...
		browserCheck: function() {
			if ($.browser.safari || $.browser.msie) {
				$("body").prepend("<p id='browser-warning'>Uh oh! This prototype may only work correctly in Firefox...</p>");
				setTimeout(function(){
